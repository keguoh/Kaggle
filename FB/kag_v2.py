...
def run_solution(h=6):
    ...
    while 1:
        ...
        time1 = int(arr[4])
        place_id = arr[5]
        hour_of_day = math.floor((time1 + 120) / (60 * h)) % (24 / h)
        log_month = math.log10(3+((time1 + 120.0) / (60 * 24 * 30)))

        ix, iy = prep_xy(x, y, grid_size_x, grid_size_y)
        grid[(ix, iy, hour_of_day)][place_id] += (1) * log_month 
        grid[(ix, iy)][place_id] += 1 * log_month
        
        if time1 < split_t:
            grid_valid[(ix, iy, hour_of_day)][place_id] += 1 * (log_month)
    ...
    f.close()
    score /= score_num
    print 'Predicted score: %r when h = %r' % (score, h)
    ...
    #     y = float(arr[2])
    #     time1 = int(arr[4])
    #     hour_of_day = math.floor((time1 + 120) / (60)) % 24
