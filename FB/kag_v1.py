...
def prep_acc(acc, range_acc):
    if acc <= 200:
        iacc = math.floor(range_acc*acc/200)
        if iacc < 0:
            iacc = 0
        if iacc >= range_acc:
            iacc = range_acc-1
...
def run_solution():
    ...
    while 1:
        ...
        iacc = prep_acc(accuracy, grid_size_acc)
        grid[(ix, iy, iacc, quarter_period_of_day)][place_id] += (1) * log_month 
        grid[(ix, iy)][place_id] += 1 * log_month
        
        if time1 < split_t:
            grid_valid[(ix, iy, iacc, quarter_period_of_day)][place_id] += 1 * (log_month)
    ...
    for arr in test_arr:
        ...
        s1 = (ix, iy, iacc, quarter_period_of_day)
        s2 = (ix, iy)
